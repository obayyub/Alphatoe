...
def get_most_recent_file(directory):
    ...
    # Adjust the regular expression to match the timestamp that's prefixed by a space and followed by a file extension
    timestamp_regex = re.compile(r"(\d{14})\.pt")

    # Debug print
    print(f"All files in {directory}: {files}")

    # Filter out any directories or files without a timestamp
    files = [f for f in files if timestamp_regex.search(f)]

    if not files:
        raise ValueError(f"No files with valid timestamps in directory: {directory}")

    # Sort the files based on the timestamps
    files.sort(
